...
const Elections = () => {
  ...
  useEffect(() => {
    ...
    const switchLayer = () => {
      Object.entries(LAYERS_TYPES).map(line =>
        map.current.setLayoutProperty(line[0], 'visibility', line[0] === activeLayer ? 'visible' : 'none')
      )
    }
  ...
  useEffect(() => {
    ...
    const contentParts = [
      ...
      `<div class="election-name">${filterValues.election} ${filterValues.year} - ${
        filterValues.round === ELECTION_ROUND_FIRST
          ? ELECTION_ROUND_LABELS[ELECTION_ROUND_FIRST]
          : ELECTION_ROUND_LABELS[ELECTION_ROUND_SECOND]
      }</div>`,
    ...
    if (participation.length && results.length) {
      contentParts.push(`
                <div class="flash-info">
                    <div class="flash-div"><span class="flash-span">${participation[0].inscrits
                      .toString()
                      .replace(/\B(?=(\d{3})+(?!\d))/g, ' ')} inscrits</span></div>
                    <div class="flash-div">
                        Taux de participation: 
                        <span class="flash-span">
                            ${renderToString(
                              <Percentage>{participation[0].votants / participation[0].inscrits}</Percentage>
                            )}
                        </span>
                    </div>
                    <div class="flash-div">Blancs et nuls: <span class="flash-span">${(
                      ((participation[0].votants - participation[0].exprimes) / participation[0].votants) *
                      100
                    ).toFixed(2)}%</span></div>
                </div>`)
      ...
      contentParts.push(
        `<div>${renderToString(
          results
            ...
            .map((element, i) => <ElectionModal key={i + 1} row={element} exprimes={participation[0].exprimes} />)
        )}</div>`
      )
    } else {
  ...
  ELECTION_TYPES.forEach(election => {
    ...
    electionSelectRows.push(
      <option key={`${value}_1`} value={`${value}_1`}>{`${label}${
        election.rounds === 1 ? 'Tour unique' : '1er tour'
      }`}</option>
    )
